...
public class Guns {
	...
	public static void openGUI(final JavaPlugin plugin, String p) {
		...
		IconMenu iconm = new IconMenu("Gun Upgrades (Credits: " + credits + ")", 36, new IconMenu.OptionClickEventHandler() {
			@Override
	...
	public static int[] getPlayerGunAttributeLevels(JavaPlugin plugin, String p, Gun g) {
		...
		ret[1] = config.isSet(path + "speed") ? config.getInt(path + "durability") : 0;
		ret[2] = config.isSet(path + "speed") ? config.getInt(path + "shoot") : 0;
		ret[3] = config.isSet(path + "speed") ? config.getInt(path + "knockback") : 0;
		return ret;
	}
	...
	public static void openGunMainEditGUI(final JavaPlugin plugin, String p, final String g) {
		IconMenu iconm = new IconMenu("Guns", 9, new IconMenu.OptionClickEventHandler() {
			@Override
			public void onOptionClick(IconMenu.OptionClickEvent event) {
				String d = event.getName();
				Player p = event.getPlayer();
