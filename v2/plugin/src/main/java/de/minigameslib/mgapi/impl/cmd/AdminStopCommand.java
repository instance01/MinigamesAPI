...
public class AdminStopCommand implements SubCommandHandlerInterface
{
    ...
    public List<String> onTabComplete(CommandInterface command, String lastArg) throws McException
    {
        ...
        {
            return MinigamesLibInterface.instance().getArenas(lastArg, 0, Integer.MAX_VALUE).stream().filter(a -> a.isMatch()).map(ArenaInterface::getInternalName).collect(Collectors.toList());
        }
