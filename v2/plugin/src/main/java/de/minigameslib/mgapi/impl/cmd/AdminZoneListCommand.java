 ...
 *
 */
public class AdminZoneListCommand extends AbstractPagableCommandHandler implements SubCommandHandlerInterface
{
    ...
    {
        final ArenaInterface arena = McLibInterface.instance().getContext(ArenaInterface.class);
        return arena.getZones().stream().
                skip(start).
                limit(count).
                map(s -> (ArenaZoneHandler) arena.getHandler(s)).
                map(ArenaZoneHandler::getName).
     ...
     * @author mepeisen
     */
    @LocalizedMessages(value = "cmd.mg2_admin_zone_list")
    public enum Messages implements LocalizedMessageInterface
    {
        
        /**
         * Short description of /mg2 admin zone list
         */
        @LocalizedMessage(defaultMessage = "lists arena zones")
        @MessageComment({"Short description of /mg2 admin zone list"})
        ShortDescription,
         ...
         */
        @LocalizedMessage(defaultMessage = "lists arena zones")
        @MessageComment({"Long description of /mg2 admin zone list"})
        Description,
        
        /**
         * Usage of /mg2 admin zone list
         */
        @LocalizedMessage(defaultMessage = "Usage: " + LocalizedMessage.CODE_COLOR + "/mg2 admin zone list <arena-name> [page]")
        @MessageComment({"Usage of /mg2 admin zone list"})
        Usage,
        
        /**
         * Pageable header line
         */
        @LocalizedMessage(defaultMessage = "Zones of " + LocalizedMessage.CODE_COLOR + "%1$s")
        @MessageComment(value = {"Pageable header line of /mg2 admin zone list"}, args=@Argument("arena internal name"))
        Header
